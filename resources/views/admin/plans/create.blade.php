    ...
    <div class="rounded-2xl p-6 border border-white/[0.07]" style="background: rgba(255,255,255,0.03);">
        ...
        <form action="{{ route('admin.plans.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" placeholder="Ex: Premium Mensuel"
                    class="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-4 py-3 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-purple-500/50 focus:bg-white/[0.06] transition">
            </div>
            ...
            <div>
                ...
                <input type="number" name="price" value="{{ old('price') }}" placeholder="Ex: 5000" min="0"
                    class="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-4 py-3 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-purple-500/50 focus:bg-white/[0.06] transition">
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Description</label>
                <textarea name="description" rows="3" placeholder="Décrivez les avantages de ce plan..."
                    class="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-4 py-3 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-purple-500/50 focus:bg-white/[0.06] transition resize-none">{{ old('description') }}</textarea>
            </div>
